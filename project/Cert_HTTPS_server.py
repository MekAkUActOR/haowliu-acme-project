...
#     runserver = Thread(
#         target=lambda: server.run(host="0.0.0.0", port=5001, threaded=True, debug=False, ssl_context=(cert, key)))
#     runserver.start()
...
class Cert_HTTPS_server:
    ...
    def start_server(self, key, cert):
        self.server.run(host="0.0.0.0", port=5001, ssl_context=(cert, key))
